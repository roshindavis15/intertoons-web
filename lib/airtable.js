export async function fetchAirtable(tableIdOrName, options = {}) {
  const url = `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/${tableIdOrName}`;
  
  const res = await fetch(url, {
    headers: {
      Authorization: `Bearer ${process.env.AIRTABLE_API_TOKEN}`,
    },
    ...options,
    next: { revalidate: 0 }, // Disable Next.js cache
    cache: 'no-store',       // Standard fetch no-cache
  });

  if (!res.ok) {
    throw new Error(`Failed to fetch Airtable data for "${tableIdOrName}": ${res.statusText}`);
  }

  return res.json();
}

/**
 * Fetches the master record for the home page.
 */
export async function getHomeData() {
  const data = await fetchAirtable('tblcseElytztktORZ');
  // Look for the consolidated "Home" record
  const homeRecord = data.records.find(r => r.fields['Page Title'] === 'Home') || data.records[0];
  return homeRecord?.fields || {};
}

async function getHomeRecord() {
  const data = await fetchAirtable('tblcseElytztktORZ');
  // Look for the consolidated "Home" record
  return data.records.find(r => r.fields['Page Title'] === 'Home') || data.records[0];
}

export async function getHeroData() {
  const homeRecord = await getHomeRecord();
  if (!homeRecord) return {};

  const heroIds = homeRecord.fields['Featured Hero'] || [];
  if (heroIds.length === 0) {
    // Fallback: Fetch the first record from Hero Sections if no link exists
    const data = await fetchAirtable('Hero Sections');
    return data.records[0]?.fields || {};
  }

  const data = await fetchAirtable('Hero Sections');
  const heroRecord = data.records.find(r => r.id === heroIds[0]);
  return heroRecord?.fields || {};
}

async function getLinkedData(fieldName, tableName) {
  const homeRecord = await getHomeRecord();
  if (!homeRecord) return [];

  const linkedIds = homeRecord.fields[fieldName] || [];
  if (linkedIds.length === 0) return [];

  const data = await fetchAirtable(tableName);
  // Filter for records linked to the home page and sort by Display Order or sort order
  return data.records
    .filter(record => linkedIds.includes(record.id))
    .map(record => ({ id: record.id, ...record.fields }))
    .sort((a, b) => {
      const orderA = Number(a['sort order'] || a['Sort Order'] || a['Display Order'] || 0);
      const orderB = Number(b['sort order'] || b['Sort Order'] || b['Display Order'] || 0);
      return orderA - orderB;
    });
}

export async function getServicesData() {
  return getLinkedData('Featured Services', 'Services');
}

export async function getProductsData() {
  return getLinkedData('Featured Products', 'Product');
}

export async function getTestimonialsData() {
  return getLinkedData('Featured Testimonials', 'Testimonials');
}

export async function getTechnologiesData() {
  return getLinkedData('Featured Technologies', 'Technologies');
}

export async function getAchievementsData() {
  return getLinkedData('Featured Milestones', 'Milestones');
}

export async function getAwardsData() {
  return getLinkedData('Featured Awards', 'Awards');
}

export async function getTeamData() {
  try {
    const data = await fetchAirtable('Team');
    if (!data.records) return [];

    return data.records
      .filter(record => record.fields['title'] && record.fields['status'] === 'Published')
      .map(record => ({
        id: record.id,
        name: record.fields['title'],
        role: record.fields['short title'] || '',
        bio: record.fields['short description'] || '',
        image: record.fields['featured image']?.[0]?.url || null,
        linkedin: record.fields['LinkedIn URL'] || null,
        twitter: record.fields['Twitter URL'] || null,
        sortOrder: record.fields['sort order'] || 999,
      }))
      .sort((a, b) => a.sortOrder - b.sortOrder);
  } catch (error) {
    console.error('Error fetching team data:', error);
    return [];
  }
}

export async function getHeaderData() {
  try {
    // 1. Fetch all navigation items from the Header table
    const headerData = await fetchAirtable('Header'); // tblDQmPZBlANIgbc1
    if (!headerData.records) return [];

    const navItems = headerData.records
      .map(record => ({
        id: record.id,
        title: record.fields['Title'],
        slug: record.fields['URL Slug'] || '/',
        order: record.fields['Display Order'] ?? 0,
        // Check if there's a link to Services (if we eventually use the link field)
        serviceIds: record.fields['Services'] || []
      }))
      .sort((a, b) => a.order - b.order);

    // 2. Enhance "Services" item with dropdown children
    // We'll fetch all published services to show in the dropdown
    const servicesData = await fetchAirtable('Services'); // tblSF57auuRbXky0U
    const allServices = servicesData.records
      .filter(r => r.fields['status'] === 'Published' || !r.fields['status'])
      .map(r => {
        const title = r.fields['title'];
        const slug = r.fields['slug'];
        
        return {
          title: title,
          slug: slug ? `/services/${slug}` : '#'
        };
      })
      .sort((a, b) => (a['sort order'] || 0) - (b['sort order'] || 0));

    // Attach services to the "Services" nav item
    return navItems.map(item => {
      if (item.title === 'Services') {
        return { ...item, children: allServices };
      }
      return item;
    });
  } catch (error) {
    console.error('Error fetching header data:', error);
    return [];
  }
}

export async function getServiceBySlug(slug) {
  try {
    const data = await fetchAirtable('Services');
    const serviceRecord = data.records.find(r => {
      const apiSlug = r.fields['slug'];
      const titleSlug = r.fields['title']?.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
      return apiSlug === slug || titleSlug === slug;
    });

    if (!serviceRecord) return null;

    const serviceData = {
      id: serviceRecord.id,
      ...serviceRecord.fields
    };

    // 1. Resolve Hero Link
    const heroIds = serviceData['Featured Hero (Link)'] || [];
    if (heroIds.length > 0) {
      try {
        const heroData = await fetchAirtable(`Hero Sections/${heroIds[0]}`);
        serviceData.hero = heroData.fields;
      } catch (heroError) {
        console.error('Error fetching linked hero data:', heroError);
      }
    }

    // 2. Resolve Portfolio Links
    const portfolioIds = serviceData['Portfolio'] || [];
    if (portfolioIds.length > 0) {
      try {
        // Fetch all portfolio records and filter for those linked to this service
        const portfolioData = await fetchAirtable('Portfolio');
        serviceData.portfolio = portfolioData.records
          .filter(r => portfolioIds.includes(r.id))
          .map(r => ({ id: r.id, ...r.fields }))
          .sort((a, b) => (a['sort order'] || 0) - (b['sort order'] || 0));
      } catch (portfolioError) {
        console.error('Error fetching linked portfolio data:', portfolioError);
        serviceData.portfolio = [];
      }
    } else {
      serviceData.portfolio = [];
    }

    // 3. Resolve Service Features List Links
    const featureIds = serviceData['Service Features'] || serviceData['Service Features List'] || serviceData['Featured Services'] || [];
    if (featureIds.length > 0) {
      try {
        // Try Service Features table first
        const featureData = await fetchAirtable('Service Features');
        let mappedFeatures = featureData.records
          .filter(r => featureIds.includes(r.id))
          .map(r => ({ id: r.id, ...r.fields }));
        
        // Fallback: If no matches found, the IDs might belong to the Services table (linking to other services)
        if (mappedFeatures.length === 0) {
          const servicesData = await fetchAirtable('Services');
          mappedFeatures = servicesData.records
            .filter(r => featureIds.includes(r.id))
            .map(r => ({ id: r.id, ...r.fields }));
        }
        
        serviceData.features = mappedFeatures.sort((a, b) => {
          const orderA = Number(a['sort order'] || a['Sort Order'] || a['Display Order'] || 0);
          const orderB = Number(b['sort order'] || b['Sort Order'] || b['Display Order'] || 0);
          return orderA - orderB;
        });
      } catch (featureError) {
        console.error('Error fetching linked features data:', featureError);
        serviceData.features = [];
      }
    } else {
      serviceData.features = [];
    }

    // 4. Resolve Milestones Links
    const milestoneIds = serviceData['Milestones'] || serviceData['Featured Milestones'] || serviceData['Service Milestones'] || [];
    if (milestoneIds.length > 0) {
      try {
        const milestoneData = await fetchAirtable('Milestones');
        serviceData.milestones = milestoneData.records
          .filter(r => milestoneIds.includes(r.id))
          .map(r => ({ id: r.id, ...r.fields }));
      } catch (milestoneError) {
        console.error('Error fetching linked milestones data:', milestoneError);
        serviceData.milestones = [];
      }
    } else {
      serviceData.milestones = [];
    }

    // 5. Resolve Testimonials Links
    const testimonialIds = serviceData['Service Testimonials'] || serviceData['Featured Testimonials'] || serviceData['Testimonials'] || [];
    if (testimonialIds.length > 0) {
      try {
        const testimonialData = await fetchAirtable('Testimonials');
        serviceData.testimonials = testimonialData.records
          .filter(r => testimonialIds.includes(r.id))
          .map(r => ({ id: r.id, ...r.fields }));
      } catch (testimonialError) {
        console.error('Error fetching linked testimonials data:', testimonialError);
        serviceData.testimonials = [];
      }
    } else {
      serviceData.testimonials = [];
    }

    // 6. Resolve Technologies Links
    const techIds = serviceData['Service Technologies'] || serviceData['Featured Technologies'] || serviceData['Technologies'] || [];
    if (techIds.length > 0) {
      try {
        const techData = await fetchAirtable('Technologies');
        serviceData.technologies = techData.records
          .filter(r => techIds.includes(r.id))
          .map(r => ({ id: r.id, ...r.fields }));
      } catch (techError) {
        console.error('Error fetching linked technologies data:', techError);
        serviceData.technologies = [];
      }
    } else {
      serviceData.technologies = [];
    }

    return serviceData;
  } catch (error) {
    console.error(`Error fetching service by slug (${slug}):`, error);
    return null;
  }
}

export async function getServiceFeatures(slug) {
  try {
    const data = await fetchAirtable('Service Features');
    return data.records
      .filter(r => r.fields['Parent Service Slug'] === slug)
      .map(r => ({
        id: r.id,
        ...r.fields
      }))
      .sort((a, b) => (a['Sort Order'] || 0) - (b['Sort Order'] || 0));
  } catch (error) {
    console.error(`Error fetching service features for slug (${slug}):`, error);
    return [];
  }
}

export async function getPortfolioByServiceSlug(slug) {
  try {
    const data = await fetchAirtable('Portfolio');
    return data.records
      .filter(r => r.fields['Parent Service Slug'] === slug && r.fields['status'] === 'Published')
      .map(r => ({
        id: r.id,
        ...r.fields
      }))
      .sort((a, b) => (a['sort order'] || 0) - (b['sort order'] || 0));
  } catch (error) {
    console.error(`Error fetching portfolio for slug (${slug}):`, error);
    return [];
  }
}

export async function getPortfolioPageData() {
  try {
    const heroesResponse = await fetchAirtable('Hero Sections');
    const heroRecord = heroesResponse.records.find(r => r.fields['Name'] === 'Portfolio') 
      || heroesResponse.records.find(r => r.fields['Subtitle'] === 'Our Portfolio')
      || heroesResponse.records[0];
      
    if (!heroRecord) {
      return { hero: {}, projects: [] };
    }

    const heroFields = heroRecord.fields;

    // 1. Resolve Linked Milestones
    let milestones = [];
    const milestoneIds = heroFields['Milestones'] || [];
    if (milestoneIds.length > 0) {
      const milestonesResponse = await fetchAirtable('Milestones');
      milestones = milestonesResponse.records
        .filter(r => milestoneIds.includes(r.id))
        .map(r => ({
          id: r.id,
          title: r.fields['Title'] || '',
          count: r.fields['Count'] || '',
          icon: r.fields['Icon']?.[0]?.url || null,
          sortOrder: Number(r.fields['sort order'] || 0)
        }))
        .sort((a, b) => a.sortOrder - b.sortOrder);
    }

    // 2. Resolve Linked Portfolio Projects
    let projects = [];
    const projectIds = heroFields['Portfolio'] || [];
    if (projectIds.length > 0) {
      // Fetch all Technologies to resolve linked icons
      const technologiesResponse = await fetchAirtable('Technologies');
      const techMap = {};
      if (technologiesResponse && technologiesResponse.records) {
        technologiesResponse.records.forEach(r => {
          techMap[r.id] = {
            id: r.id,
            name: r.fields['title'] || r.fields['Title'] || r.fields['Name'] || '',
            url: r.fields['icon']?.[0]?.url || r.fields['Icon']?.[0]?.url || null
          };
        });
      }

      const portfolioResponse = await fetchAirtable('Portfolio');
      projects = portfolioResponse.records
        .filter(r => projectIds.includes(r.id))
        .map(r => {
          const techIds = r.fields['Technologies'] || r.fields['technologies'] || [];
          const projectTechs = techIds
            .map(id => techMap[id])
            .filter(Boolean);

          return {
            id: r.id,
            title: r.fields['title'] || r.fields['Title'] || '',
            category: r.fields['category'] ? r.fields['category'].trim() : (r.fields['short title'] || r.fields['Short Title'] || '').trim(),
            image: r.fields['featured image']?.[0]?.url || r.fields['Featured Image']?.[0]?.url || null,
            tags: r.fields['category'] ? r.fields['category'].split('|').map(t => t.trim()) : (r.fields['Tags'] ? r.fields['Tags'].split('|').map(t => t.trim()) : []),
            slug: r.fields['slug'] || '',
            sortOrder: Number(r.fields['sort order'] || 0),
            technologies: projectTechs
          };
        })
        .sort((a, b) => a.sortOrder - b.sortOrder);
    }

    return {
      hero: {
        title: heroFields['Title'] || "Our Work. Your Success",
        subtitle: heroFields['Subtitle'] || "OUR PORTFOLIO",
        description: heroFields['Description'] || "",
        image: heroFields['Desktop Image']?.[0]?.url || null,
        milestones: milestones
      },
      projects: projects
    };
  } catch (error) {
    console.error('Error fetching portfolio page data:', error);
    return { hero: {}, projects: [] };
  }
}
