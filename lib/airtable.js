...
export async function getHeroData() {
  ...
  const data = await fetchAirtable('Homepage%20Content');
...
async function getLinkedData(sectionName, fieldName, tableName) {
  ...
  const homepageData = await fetchAirtable('Homepage%20Content');
...
export async function getProductsData() {
  return getLinkedData('Products', 'Homepage Products', 'Products');
}
